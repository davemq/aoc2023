...
# For each game (line), we want to track
# - ID
# - max red
# - max green
# - max blue
#
# Then compare to specified number of red, green, and blue cubes.

# For each line
# - split into "Game <ID>: " and the rest of the string
# - parse out game ID
# - split rest of line into games
# - split games into colors and values
    ...
    for g in games:
        ...
        for c in cubes:
            ...
            if value > maxima[color]:
                maxima[color] = value
    if maxima['red'] <= bag['red'] and maxima['green'] <= bag['green'] \
       and maxima['blue'] <= bag['blue']:
        sum += id
